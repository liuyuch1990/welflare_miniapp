...
<script>
	...
	export default {
		...
		methods: {
			...
			var params = {
				"url": getProvince.url,
				"content-type": getProvince.contentType,
				payload: {}
			}
			this.request.getRequest(params).then(res => {
				this.districtsObj.provinces = res
				this.loadCities(res[0].code)
			})
			...
			loadCities(id) {
				...
				var params = {
					"url": getCity.url+id,
					"content-type": getCity.contentType,
					payload: {}
				}
				this.request.getRequest(params).then(res => {
					this.districtsObj.cities  = res
					this.loadAreas(res[0].code)
				})
			...
			loadAreas(id) {
				...
				var params = {
					"url": getArea.url+id,
					"content-type": getArea.contentType,
					payload: {}
				}
				this.request.getRequest(params).then(res => {
					this.districtsObj.areas = res
				})
